...
> **业务专用指南**：提供账单拟退（预退费）场景的完整配置示例和步骤

---

## 目的

将Jarvis的账单拟退操作同步至HIS系统，拟退是指已收费的账单进行退费，在HIS侧生成拟退记录，为后续正式退费做准备。

---

## 场景识别

当客户提到「拟退账单」「预退费」「退费审批」「账单退费申请」时，明确为账单拟退场景。
...
| hisOrderIds | `chagPatientSheetList[].presId` | HIS处方ID数组（支持批量拟退） |
...
- `chagPatientSheetList` - 待拟退账单列表（数组，支持批量）
- `chagPatientSheetList[].presId` - HIS处方ID（拟退标识）
- `patientInfo.hisPatientId` - HIS患者档案号（必填）
- `doctorInfo.number` - 操作员编号
- `doctorInfo.userName` - 操作员姓名
- `chagPatientSheetDetail` - 拟退项目明细（负数金额）
